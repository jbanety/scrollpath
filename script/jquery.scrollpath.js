...
(function($, window, document, undefined) {
        ...
        methods = {
            ...
            init: function(options) {
                if (this.length > 1 || isInitialized) {
                    $.error("jQuery.scrollPath can only be initialized on *one* element *once*");
                }
                ...
                //  instead
                $.easing.linearSp = function(x, t, b, c, d) {
                    return x * (c - b);
                };
            ...
            scrollTo: function(name, duration, easing, callback) {
                ...
                if (destination === undefined) {
                    $.error("jQuery.scrollPath could not find scroll target with name '" + name + "'");
                }
            ...
            getScrollHandler: function() {
                ...
                return function(e) {
                    // Copied from below!!
                    var scrollDelta = e.originalEvent.wheelDelta || -e.originalEvent.detail, dir = scrollDelta / ( Math.abs(scrollDelta) );
                    e.preventDefault();
        ...
        this.rotate = function(radians, options) {
            ...
            if (!HAS_TRANSFORM_SUPPORT) {
                if (settings.name || settings.callback) {
                    ...
                    // so they don't get lost in browsers without rotation support
                    this.moveTo(xPos, yPos, {
                        callback: settings.callback, name: settings.name
                    });
                }
                return this;
            }
    ...
    function initScrollBar() {
        ...
        scrollBar = $("<div>").addClass("sp-scroll-bar").on("mousedown", function(e) {
        ...
        scrollHandle = $("<div>").addClass("sp-scroll-handle").on({
                click: function(e) {
                    e.preventDefault();
                    return false;
    ...
    function touchHandler(e) {
        ...
        var touches = e.originalEvent.changedTouches, lastTouchX = touches[0].clientX, lastTouchY = touches[0].clientY, touchId = touches[0].identifier;
        ...
        var touchMove = function(e) {
            ...
            var touches = e.originalEvent.changedTouches, touch = findTouchFromId(touches, touchId);

            if (!touch) {
                return;
            } // "our" touch hasn't changed
            ...
            var deltaX = (touch.clientX - lastTouchX) * settings.touchDistanceAmplificationFactor, deltaY = (touch.clientY - lastTouchY) * settings.touchDistanceAmplificationFactor, greatestDelta = (Math.abs(deltaX) > Math.abs(deltaY)) ? deltaX : deltaY, direction = (greatestDelta >= 0) ? -1 : 1; // Intentionally inverted to match expected 'drag' behaviour
        ...
        $(document).on({
            'touchmove.sp-touchsupport': touchMove, 'touchend.sp-touchsupport': function(e) {
    ...
    function dragScrollHandler(e) {
        var dragStep, y = e.clientY - scrollBar.offset().top;
    ...
    function animateSteps(steps, duration, easing, callback) {
        ...
        var frames = ( duration / 1000 ) * FPS, startStep = step, currentFrame = 0, easedSteps, nextStep, interval = setInterval(function() {
    ...
    function scrollToStep(stepParam, fromAnimation) {
        ...
        if (cb && stepParam !== step && !isAnimating) {
            cb();
        }
        step = stepParam;
    }
    ...
    function wrapStep(wStep) {
        if (settings.wrapAround) {
            if (isAnimating) {
                while (wStep < 0) {
                    wStep += pathList.length;
                }
                while (wStep >= pathList.length) {
                    wStep -= pathList.length;
                }
            } else {
                if (wStep < 0) {
                    wStep = pathList.length - 1;
                }
                if (wStep >= pathList.length) {
                    wStep = 0;
                }
            }
        } else {
    ...
    function sectorAngle(start, end, ccw) {
        var nStart = normalizeAngle(start), nEnd = normalizeAngle(end), diff = Math.abs(nStart - nEnd), invDiff = Math.PI * 2 - diff;

        if (( ccw && nStart < nEnd ) || ( !ccw && nStart > nEnd ) || ( nStart === nEnd && start !== end ) // Special case *
        ) {
            return invDiff;
        }
